...
public class IntervalSearch
{
	...
	Interval findInterval(T[] sortedData, TreeSet<T> targetValues)
	{
		...
		if (sortedData.length > 0 && targetValues.size() > 0)
		{
			...
			//values are part of TreeSet, and so are ordered and non-repeating
			...
			if(sortedData[sortedData.length-1].compareTo(smallestTarget) <= 0)
			{
				leftBoundary = sortedData.length;
			}
			else if(sortedData[0].compareTo(smallestTarget) <= 0)
			{
				int left = 0;
			...
			else if(sortedData[sortedData.length-1].compareTo(largestTarget) >= 0)
			{
				int left = (leftBoundary != -1 ? leftBoundary : 0);
				int right = sortedData.length - 1;
				int mid = left + (right - left)/2;
				boolean found = false;
